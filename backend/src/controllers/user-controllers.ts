...
export const userSignup = async (
  ...
  try {
    ...
    return res.status(201).json({ message: "OK", id: user._id.toString() }); // status 200: OK - send json object with users
  } catch (error) {
    console.log(error);
    return res.status(500).json({ message: "ERROR", cause: error.message }); // status 500: Internal Server Error - send json object with error message
  }
...
export const userLogin = async (
  ...
  try {
    ...
    return res.status(200).json({ message: "OK", id: user._id.toString() }); // status 200: OK - send json object with users
  } catch (error) {
    console.log(error);
    return res.status(500).json({ message: "ERROR", cause: error.message }); // status 500: Internal Server Error - send json object with error message
  }
